...
class gas_turbine(object):
    ...
    def pie_en(self):
        fig = plt.figure(num="Energy Distribution", figsize=(10, 8)) 
        ...
        wedges, texts, autotexts = ax.pie(sizes,colors=colors,autopct='%1.1f%%',startangle=90,pctdistance=0.7,radius=0.5,textprops={'fontsize': 14, 'fontweight': 'bold'})
        ax.legend(wedges,labels,loc='upper center',bbox_to_anchor=(0.5, -0.1),fontsize=13,ncol=2,frameon=False)

        ax.set_title('ENERGY DISTRIBUTION', fontsize=16, fontweight='bold')  
        ax.axis('equal')
        plt.tight_layout()  
    ...
    def pie_ex(self):
        fig = plt.figure(num="Exergy Distribution", figsize=(10, 8))
        ax = fig.add_subplot(111)

        labels = [f'Mechanical losses {self.loss_mec * 1e-6:.2f} MW',f'Rotor exergy losses {self.loss_rotex * 1e-6:.2f} MW',f'Combustion exergy losses {self.loss_combex * 1e-6:.2f} MW',f'Exhaust exergy losses {self.loss_echex * 1e-6:.2f} MW',f'Effective exergy power {self.P_e * 1e-6:.2f} MW']
        ...
        ax.set_title('EXERGY DISTRIBUTION', fontsize=16, fontweight='bold')
        ax.axis('equal')  
        plt.tight_layout()
