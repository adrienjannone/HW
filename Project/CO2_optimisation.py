...
eta_turb = 0.93                      # Turbine adiabatic efficiency (isentropic)
...
if ETATURB:
    ...
    m, c = np.polyfit(results['eta_turb'], results['total_energy'], 1)

    print(f"Slope (Total power vs $\\eta_{{turb}}$): {m:.2f}")

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    ax1.plot(results['eta_turb'], results['total_energy'], 'r-o', label='Total power (W)')
    ax1.set_xlabel('Efficiency of the turbine, $\\eta_{turb}$ [-]', fontsize=LABEL_FONTSIZE)
    ax1.set_ylabel('Total power used [W]', fontsize=LABEL_FONTSIZE)
    ax1.set_title('Sensibility of the total power used with $\\eta_{turb}$', fontsize=TITLE_FONTSIZE)
    ...
    ax2.set_xlabel('Efficiency of the turbine,, $\\eta_{turb}$ [-]', fontsize=LABEL_FONTSIZE)
    ax2.set_ylabel('Efficiency Rotex (Cycle), $\\eta_{rotex}$ [%]', fontsize=LABEL_FONTSIZE)
    ax2.set_title('$\\eta_{rotex}$ en fonction de $\\eta_{turb}$', fontsize=TITLE_FONTSIZE)
...
# Analyze the effect of pinch points
if PINCHES:
    pinch_TES_values = [2.5, 3.5, 5, 6, 7.5] 
    results_pinch = {'pinch_TES': [], 'total_energy': [], 'eta_rotex': [], 'm_dot_CO2': []}

    TITLE_FONTSIZE = 16
    LABEL_FONTSIZE = 14
    TICK_FONTSIZE = 12

    for pinch_TES in pinch_TES_values:
        params_iter = params.copy()
        params_iter['pinch_TES'] = pinch_TES
        dome = CO2_battery(inputs, params_iter, False)
        results_tuple = dome.evaluate()
        m_dot_CO2 = results_tuple[6]
        eta_rotex = results_tuple[12]
        total_energy = results_tuple[-1] 
        results_pinch['pinch_TES'].append(pinch_TES)
        results_pinch['total_energy'].append(total_energy)
        results_pinch['eta_rotex'].append(eta_rotex * 100) 
        results_pinch['m_dot_CO2'].append(m_dot_CO2)

    m_pinch, c_pinch = np.polyfit(results_pinch['pinch_TES'], results_pinch['total_energy'], 1)
    
    print(f"Slope (Total power vs Pinch point): {m_pinch:.2f}")

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    ax1.plot(results_pinch['pinch_TES'], results_pinch['total_energy'], 'b-o', label='Total energy (W)')
    ax1.set_xlabel('Pinch point in TES [K]', fontsize=LABEL_FONTSIZE)
    ax1.set_ylabel('Total energy used [W]', fontsize=LABEL_FONTSIZE)
    ax1.set_title('Sensibility of the total energy used with pinch point in TES', fontsize=TITLE_FONTSIZE)
    ax1.tick_params(axis='both', which='major', labelsize=TICK_FONTSIZE)
    ax1.grid(True)
    ax1.legend(fontsize=TICK_FONTSIZE)

    ax2.plot(results_pinch['pinch_TES'], results_pinch['eta_rotex'], 'm-^', label='Efficiency Rotex (%)')
    ax2.set_xlabel('Pinch point in TES [K]', fontsize=LABEL_FONTSIZE)
    ax2.set_ylabel('Efficiency Rotex (Cycle), $\\eta_{rotex}$ [%]', fontsize=LABEL_FONTSIZE)
    ax2.set_title('$\\eta_{rotex}$ en fonction du pinch point dans le TES', fontsize=TITLE_FONTSIZE)
    ax2.tick_params(axis='both', which='major', labelsize=TICK_FONTSIZE)
    ax2.grid(True)
    ax2.legend(fontsize=TICK_FONTSIZE)
